...
func TestSetupRouter(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "flight search endpoint exists",
			method:         http.MethodPost,
...
func TestAPIv1FlightSearchRoute(t *testing.T) {
	e := echo.New()
	cfg := &config.Config{
		Server: config.ServerConfig{
			Port:         8080,
	...
	t.Run("POST /api/v1/flights/search route exists", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodPost, "/api/v1/flights/search", nil)
		...
		// Should be 400 for invalid request (no body)
		assert.Equal(t, http.StatusBadRequest, rec.Code)
	})

	t.Run("GET /api/v1/flights/search not allowed", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodGet, "/api/v1/flights/search", nil)
		rec := httptest.NewRecorder()

		e.ServeHTTP(rec, req)

		// Echo returns 404 when no route matches the method, not 405
		// This is expected behavior since only POST is registered
...
func TestCORSMiddleware(t *testing.T) {
	...
	t.Run("CORS headers present in API routes", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodOptions, "/api/v1/flights/search", nil)
		...
		// Verify CORS headers
		assert.NotEmpty(t, rec.Header().Get(echo.HeaderAccessControlAllowOrigin))
		assert.Contains(t, rec.Header().Get(echo.HeaderAccessControlAllowMethods), "POST")
		assert.Contains(t, rec.Header().Get(echo.HeaderAccessControlAllowHeaders), echo.HeaderContentType)
	})

	t.Run("CORS headers in POST response", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodPost, "/api/v1/flights/search", nil)
		req.Header.Set(echo.HeaderOrigin, "http://localhost:3000")
		req.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)
		rec := httptest.NewRecorder()
...
func TestRequestIDMiddleware(t *testing.T) {
	...
	t.Run("Request ID generated for all requests", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodGet, "/health", nil)
		rec := httptest.NewRecorder()
	...
	t.Run("Request ID generated for API routes", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodPost, "/api/v1/flights/search", nil)
		req.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)
		rec := httptest.NewRecorder()
